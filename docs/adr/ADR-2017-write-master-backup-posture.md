...
**Governed changes since `1ad881cab`:** `src/services/github_sync_service.rs` reads the corpus through the `CorpusSource` port (ADR-2114) — a mounted vault directory by default, the GitHub repository when `CORPUS_SOURCE=github` — and selects ontology nodes by the declared frontmatter `type` rather than `owl_class_iri.is_some()`. The type keeps its historical name. `data_reconciliation.rs` and both backup scripts are unchanged.

**Decision unaffected, one term refined.** The invariant — each data class has one authoritative store, the Oxigraph `:assert` projection is disposable and regenerated by full `CLEAR + INSERT DATA` from the corpus upstream, never hand-edited — holds exactly as before; `clear_graph()` on `force_full_sync` and the atomic rebuild are unchanged. What ADR-2114 changes is *which* upstream: read "the GitHub upstream" and "re-sync from GitHub" above as "the configured `CorpusSource` (the vault directory by default, GitHub when selected)". The write-master for the corpus is therefore the vault, whose own authority chain is ADR-2112/ADR-2114. `verified_commit` moved to the CI-repair commit.
