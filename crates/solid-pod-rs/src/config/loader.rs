...
impl ConfigLoader {
    ...
    pub async fn load(mut self) -> Result<ServerConfig, PodError> {
        // If no Defaults source was registered, inject it so the
        ...
        let mut had_storage_root_with_memory = false;
        ...
        for source in &self.sources {
            ...
            if let ConfigSource::EnvVars = source {
                ...
                if type_is_memory && root_was_set {
                    had_storage_root_with_memory = true;
                    self.warnings.push(
        ...
        // Reference to silence unused warning when tracing is disabled.
        let _ = had_storage_root_with_memory;
