...
///
/// The returned value is a `serde_json::Value::Object` (or null for
/// the empty case) that is merged into the accumulator by
/// [`merge_json`] in precedence order.
    ...
    for (k, v) in obj {
        match k.as_str() {
            "host" | "port" | "base_url" | "baseUrl" => {
                ...
                let key = if k == "baseUrl" { "base_url".to_string() } else { k };
                server.insert(key, v);
            }
            _ => {
