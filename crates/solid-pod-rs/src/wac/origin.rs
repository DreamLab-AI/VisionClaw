...
impl OriginPattern {
    ...
    pub fn parse(s: &str) -> Option<Self> {
        ...
        if origin.as_str() != trimmed.to_ascii_lowercase() {
            // Allow case-normalisation differences but reject any
            // other canonicalisation gap.
    ...
    pub fn matches(&self, origin: &Origin) -> bool {
        match self {
            ...
            OriginPattern::Wildcard { scheme, suffix } => {
                ...
                // front. Preserve port exactly.
                //
                // Strip the port from the comparison target and the
                // pattern suffix so `https://*.example.org` matches
                // `https://app.example.org:8443` regardless of port
                // (ports are not part of the "which suffix" question).
                ...
                // Must be strictly a subdomain: "app.example.org" ends
                // with ".example.org", but "example.org" alone must not.
