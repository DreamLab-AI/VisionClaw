...
impl FsBackend {
    ...
    fn resolve(&self, path: &str) -> Result<PathBuf, PodError> {
        ...
        let canonical_root = self.root.to_path_buf();
...
impl Storage for FsBackend {
    ...
    async fn head(&self, path: &str) -> Result<ResourceMeta, PodError> {
        ...
        // We need bytes to compute ETag; read-once is acceptable for
        // HEAD on small resources. For a production FS backend we
        // would cache ETags alongside sidecars.
    ...
    async fn watch(
        &self,
        path: &str,
    ) -> Result<mpsc::Receiver<StorageEvent>, PodError> {
        ...
        // The watcher thread sends events over a std::sync::mpsc
        // channel; an async task forwards to the tokio mpsc.
        ...
        // If the path doesn't exist yet, watch the root directory.
        ...
        tokio::task::spawn_blocking(move || {
            // Keep watcher alive for the lifetime of the forwarder.
            let _keep = watcher;
            while let Ok(Ok(event)) = raw_rx.recv() {
                for path in &event.paths {
                    // Skip sidecar files from the event stream.
                    let s = path.to_string_lossy();
                    if s.ends_with(META_SUFFIX) {
                        continue;
                    }
                    // Derive virtual path relative to root.
                    let virt = match path.strip_prefix(root.as_path()) {
                        Ok(p) => format!("/{}", p.to_string_lossy()),
                        Err(_) => continue,
                    };
                    // Filter: event must be under the subscriber's filter root.
