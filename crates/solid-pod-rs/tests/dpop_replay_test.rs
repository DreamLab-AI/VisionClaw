...
fn build_dpop_proof(secret: &[u8], jwk: &Jwk, htu: &str, htm: &str, iat: u64, jti: &str) -> String {
    ...
    // Sign with the same HS256 path jsonwebtoken uses so the produced
    // proof is verifiable by a real consumer.
    ...
    .unwrap();
    // encode() returns full JWT — we only want its signature segment.
