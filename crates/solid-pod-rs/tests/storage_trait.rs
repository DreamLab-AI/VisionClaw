...
//!
//! Every backend shipped with the crate must pass this suite. New
//! backends should run these tests via a fresh instance.
...
async fn run_concurrent_writes(s: Arc<dyn Storage>) {
    // 100 parallel PUTs to distinct keys.
...
async fn run_watch_receives_event(s: Arc<dyn Storage>) {
    ...
    // Give the FS watcher a moment to arm.
